...
export const Screen2PetType: React.FC<Props> = ({ data, onChange, error }) => {
  ...
  return (
    <div className="flex flex-col gap-6 px-6 py-8">
      <div>
        <h2
          className="text-3xl text-gray-900 leading-snug whitespace-pre-line"
          style={{ fontFamily: '"DM Serif Display", serif' }}
        >
          {t('onboarding.screen2.title')}
        </h2>
        <p className="text-gray-400 text-sm mt-2">{t('onboarding.screen2.subtitle')}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        {PET_OPTIONS.map(({ type, photo }) => {
          const selected = data.petType === type
          const label = t(`pets.${type}.label`)
          return (
            <button
              key={type}
              type="button"
            ...
            >
              ...
              <div className={[
                ...
                selected ? 'bg-[#7C3AED]/30' : 'bg-black/10 hover:bg-black/5',
              ].join(' ')} />
