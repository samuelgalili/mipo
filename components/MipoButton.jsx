// MipoButton.jsx — MIPO Design System
import React from "react";

/**
 * MipoButton — כפתור ראשי של מערכת העיצוב
 *
 * @param {string}   label      — טקסט הכפתור
 * @param {function} onClick    — פונקציה בלחיצה
 * @param {boolean}  disabled   — מושבת
 * @param {boolean}  loading    — מצב טעינה (מציג ספינר)
 * @param {string}   variant    — primary | secondary | ghost | danger | outline
 * @param {string}   size       — sm | md | lg | xl
 * @param {node}     iconStart  — אייקון בתחילת הכפתור (ימין ב-RTL)
 * @param {node}     iconEnd    — אייקון בסוף הכפתור (שמאל ב-RTL)
 * @param {boolean}  fullWidth  — רוחב מלא
...
const MipoButton = ({
  ...
  const variants = {
    ...
    outline: [
      "bg-transparent border border-purple-300 text-purple-500",
    ...
    danger: [
      ...
      "hover:from-red-600 hover:to-rose-600 hover:shadow-xl hover:shadow-red-500/40",
    ].join(" "),
  };
